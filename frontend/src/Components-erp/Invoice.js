...
effectCount++
  ...
  const getUserData = () => {      //function to get USer data 
    ...
    setName(nae);
    const Account = localStorage("invoice")
    const {invocieNo} = Account
    let aa = invocieNo +1
    setinvoiceNo(aa)
  };
  ...
  const ddd = new Date();
   ...
   const Type = 'send'
    const res = await fetch("/create", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ invoice,Cookie,SGST,CGST,GrandTotal,BilledTo,ShippedTo,Name,status,Type,invoiceNo }),
    })
